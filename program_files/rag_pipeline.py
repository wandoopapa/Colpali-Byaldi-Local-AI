...
def _index_storage_exists(index_folder: Path, index_name: str) -> bool:
    # 실행 안정성 개선: 기존 동일 인덱스가 있으면 add_to_index 경로를 우선 사용.
    if (index_folder / "index_manifest.json").exists():
        return True
    direct = index_folder / index_name
    if direct.exists():
        return True
    return any(index_folder.glob(f"{index_name}*"))


def _normalize_indexing_error(exc: Exception) -> Exception:
    text = str(exc)
    if "already loaded" in text and "add_to_index" in text:
        return RuntimeError(
            "동일한 인덱스 이름이 이미 존재합니다. 자동으로 기존 인덱스에 이어서 추가하도록 재시도했지만 실패했습니다. 인덱스 폴더 권한/손상 여부를 확인해 주세요."
        )
...
def _call_variants_for_doc(doc_path: Path, config: PipelineConfig, add_to_index: bool) -> Iterable[dict]:
    return [
        {
...
def run_indexing(config: PipelineConfig, progress: ProgressCallback) -> None:
    ...
    index_initialized = _index_storage_exists(config.index_folder, config.index_name)
    if index_initialized:
        _notify(progress, "기존 인덱스를 확인했습니다. 이어서 문서를 추가합니다...", 8, 20)
    ...
    for idx, doc_path in enumerate(docs, start=1):
        ...
        indexed = False
        add_mode = index_initialized

        for kwargs in _call_variants_for_doc(doc_path, config, add_mode):
            try:
                ...
                # Byaldi가 기존 인덱스에 add_to_index를 요구하는 경우 자동 전환 재시도.
                text = str(exc)
                if ("already loaded" in text and "add_to_index" in text) and not add_mode:
                    add_mode = True
                    for retry_kwargs in _call_variants_for_doc(doc_path, config, add_mode):
                        try:
                            _notify(progress, f"기존 인덱스 추가 모드 재시도: {doc_path.name}", base_total + 6, 70)
                            model.index(**retry_kwargs)
                            indexed = True
                            index_initialized = True
                            break
                        except TypeError:
                            ...
                            raise _normalize_indexing_error(retry_exc) from retry_exc
                    if indexed:
                        break
