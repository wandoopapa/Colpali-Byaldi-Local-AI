...
class InstallerApp(tk.Tk):
    ...
    def _run_install(self, target: Path) -> None:
        try:
            ...
            self._set_progress("PyTorch(CUDA) 설치 준비 중...", 62, 10)
            self._install_torch_with_fallback()

            self._set_progress("라이브러리 설치 준비 중...", 78, 5)
    ...
    def _run_pip(self, cmd: list[str], base_total: float) -> None:
        creationflags = 0
        if os.name == "nt":
        ...
        while True:
            ...
            if line:
                output_lines += 1
                item_pct = min(95, 20 + output_lines)
                total_pct = min(99, base_total + output_lines * 0.2)
                self._set_progress(line.strip()[:120], total_pct, item_pct)

        if proc.returncode != 0:
            raise RuntimeError(f"라이브러리 설치 실패: {' '.join(cmd)}")

    def _install_torch_with_fallback(self) -> None:
        # 사용자 요구사항: 반드시 GPU(CUDA) 기반 설치만 진행.
        self._set_progress("CUDA용 PyTorch 설치 시도 중...", 63, 20)
        cuda_cmd = [
            sys.executable,
            "-m",
            "pip",
        ...
        self._run_pip(cuda_cmd, 63)
        self._set_progress("CUDA용 PyTorch 설치 완료", 77, 100)
    ...
    def _install_requirements(self, requirements_path: Path) -> None:
        ...
        for idx, cmd in enumerate(commands, start=1):
            ...
            self._run_pip(cmd, 78 + (idx - 1) * 10)

        self._set_progress("라이브러리 설치 완료", 98, 100)
